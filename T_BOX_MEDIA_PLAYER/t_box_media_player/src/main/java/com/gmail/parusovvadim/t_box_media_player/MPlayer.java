...
public class MPlayer extends Service implements OnCompletionListener, MediaPlayer.OnErrorListener
{
    ...
    {
        m_mediaSessionCompat = new MediaSessionCompat(this, "com.gmail.parusovvadim.t_box_media_player");
    ...
    private void createPlayList()
    {
        ...
        for(int i = 0; i < listTrack.size(); i++)
        {
            ...
            {
                metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_ART, BitmapFactory.decodeResource(getResources(), R.drawable.song_image));
            }
            metadataBuilder.putString(MediaMetadataCompat.METADATA_KEY_TITLE, title).putString(MediaMetadataCompat.METADATA_KEY_ALBUM, artist).putString(MediaMetadataCompat.METADATA_KEY_ARTIST, artist).putLong(MediaMetadataCompat.METADATA_KEY_DURATION, durationMs);
